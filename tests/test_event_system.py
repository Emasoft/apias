...
def test_basic_publish_subscribe_flow():
    ...
    assert received_events[0] is error, "Handler should receive the exact event published"
...
def test_type_safe_subscriptions():
    ...
    assert len(error_handler_calls) == 0, "ErrorEvent handler should NOT be called for StatusEvent"
...
def test_event_queuing():
    ...
    for i in range(10):
        if i % 3 == 0:
            event = StatusEvent(task_id=i, state=URLState.PROCESSING, message=f"Status {i}")
        elif i % 3 == 1:
            event = ErrorEvent(category=ErrorCategory.API_TIMEOUT, message=f"Error {i}")
        else:
            event = CircuitBreakerEvent(reason=f"Circuit breaker {i}", affected_tasks=[i])
    ...
    assert received_events == events_published, "Events should be received in FIFO order"
...
def test_get_stats_returns_correct_metrics():
    ...
    # Publish 5 events
    for i in range(5):
        bus.publish(ErrorEvent(category=ErrorCategory.PARSE_ERROR, message=f"Parse error {i}"))
...
def test_concurrent_publishing_from_worker_threads():
    ...
    stats = bus.get_stats()
    assert stats["published"] == 1000, "Should have published 1000 events from 10 workers"
...
def test_queue_full_handling():
    ...
    stats = bus.get_stats()
    assert successful_publishes == 10, "Should successfully publish exactly 10 events (queue size)"
...
def test_handler_exception_isolation():
    ...
    # Publish event
    status = StatusEvent(task_id=99, state=URLState.FAILED, message="Test failure isolation")
    bus.publish(status)
    ...
    assert len(handler2_calls) == 1, "Normal handler should still be called despite handler1 exception"
    assert handler1_calls[0] is status
    assert handler2_calls[0] is status
...
def test_dispatch_timeout():
    ...
    stats_after = bus.get_stats()
    assert stats_after["dispatched"] == count, "Dispatched count should match events processed"
    assert stats_after["pending"] == 1000 - count, "Pending should be reduced by dispatched count"
...
def test_dispatch_processes_until_queue_empty():
    ...
    assert stats["pending"] == 0, "Queue should be empty (dispatch stopped early when queue empty)"
...
def test_dispatch_returns_event_count():
    ...
    for i in range(15):
        if i % 2 == 0:
            bus.publish(StatusEvent(task_id=i, state=URLState.PROCESSING, message=f"Status {i}"))
        else:
            bus.publish(ErrorEvent(category=ErrorCategory.UNKNOWN, message=f"Error {i}"))
...
def test_error_event_creation_with_all_fields_and_defaults():
    ...
    assert event_minimal.exception_traceback is None, "exception_traceback should default to None"
...
def test_circuit_breaker_and_dialog_event_creation():
    ...
    assert circuit_minimal.affected_tasks == [], "affected_tasks should default to empty list"
    assert circuit_minimal.trigger_category is None, "trigger_category should default to None"
    assert circuit_minimal.consecutive_counts == {}, "consecutive_counts should default to empty dict"
