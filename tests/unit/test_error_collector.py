...
class TestSmartErrorStorage:
    ...
    def test_add_and_get_recent_with_maxlen_boundary(self) -> None:
        ...
        assert recent[0].message == "Connection error 149", (
            "Most recent error should be first"
        )
        assert recent[-1].message == "Connection error 50", (
            "Oldest retained error should be last"
        )

        # Verify summary shows correct counts
        summary = storage.get_summary()
        assert summary["total_recorded"] == 150, (
        ...
        assert summary["recent_count"] == 100, (
    ...
    def test_get_category_stats_returns_correct_counts(self) -> None:
        ...
        assert len(specific_stats) == 1, (
            f"Expected 1 category in specific stats, got {len(specific_stats)}"
        )
        assert specific_stats[ErrorCategory.RATE_LIMIT].count == 15, (
    ...
    def test_get_first_occurrence_returns_earliest_error(self) -> None:
        ...
        assert first.message == "Connection error 0", (
    ...
    def test_get_snapshot_returns_deep_copy(self) -> None:
        ...
        assert stats_first is not stats_second, (
        ...
        assert stats_second[ErrorCategory.CONNECTION_ERROR].count == 10, (
...
class TestCircuitBreakerV2:
    ...
    def test_consecutive_error_threshold(self) -> None:
        ...
        assert "3 consecutive API_TIMEOUT" in result.trigger_reason, (
    ...
    def test_immediate_trip_for_fatal_errors(self) -> None:
        ...
        assert "Fatal error" in result.trigger_reason, (
    ...
    def test_per_category_independence(self) -> None:
        ...
        assert not breaker2.is_triggered, (
    ...
    def test_get_status_returns_comprehensive_state(self) -> None:
        ...
        assert breaker.trigger_context is None, (
        ...
        assert context.triggering_error is not None, (
            "Context triggering_error should be set"
        )
        assert ErrorCategory.CONNECTION_ERROR in context.consecutive_counts, (
...
"""
        ...
        try:
            ...
            # Verify thresholds
            assert config.thresholds[ErrorCategory.RATE_LIMIT] == 5, (
                "Expected RATE_LIMIT threshold 5"
            )
            assert config.thresholds[ErrorCategory.API_TIMEOUT] == 10, (
                "Expected API_TIMEOUT threshold 10"
            )
            assert config.thresholds[ErrorCategory.CONNECTION_ERROR] == 7, (
                "Expected CONNECTION_ERROR threshold 7"
            )

            # Verify immediate trip categories
            assert ErrorCategory.QUOTA_EXCEEDED in config.immediate_trip, (
                "Expected QUOTA_EXCEEDED in immediate_trip"
            )
            assert ErrorCategory.AUTHENTICATION in config.immediate_trip, (
                "Expected AUTHENTICATION in immediate_trip"
            )

            # Verify storage config
            assert config.max_recent_errors == 200, (
                f"Expected max_recent_errors 200, got {config.max_recent_errors}"
            )
            assert config.max_total_errors == 100000, (
...
class TestErrorCollector:
    ...
    def test_record_error_triggers_circuit_breaker(self) -> None:
        ...
        for i in range(3):
            ...
            if i < 2:
                ...
                assert "3 consecutive RATE_LIMIT" in result.trigger_reason, (
        ...
        assert "3 consecutive RATE_LIMIT" in circuit_event.reason, (
    ...
    def test_thread_safety_concurrent_error_recording(self) -> None:
        ...
        stats = collector.get_stats()
        assert stats["total_recorded"] == total_expected, (
    ...
    def test_exception_enrichment(self) -> None:
        ...
        assert error_event.exception_traceback is not None, (
            "Exception traceback should be populated"
        )
        assert "Test exception with traceback" in error_event.exception_traceback, (
            "Traceback should contain exception message"
        )
        assert "ValueError" in error_event.exception_traceback, (
        ...
        assert recent[0].exception_traceback is not None, (
    ...
    def test_get_recent_errors_returns_list(self) -> None:
        ...
        assert all(isinstance(e, ErrorEvent) for e in all_recent), (
