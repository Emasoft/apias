...
from apias.batch_tui import URLTask, URLState
...
def test_status_pipeline_initialization():
    ...
    assert isinstance(pipeline._critical_event, threading.Event), "Should have critical event flag"
    assert not pipeline._critical_event.is_set(), "Critical event flag should start unset"
...
def test_initialize_tasks_creates_url_task_objects():
    ...
    for task_id, expected_url in enumerate(urls, start=1):
        ...
        assert task.state == URLState.PENDING, f"Task {task_id} should start in PENDING state"
...
def test_on_status_event_updates_task_state():
    ...
    assert task.status_message == "Processing chunk 2/5", "Status message should be updated"
...
def test_on_status_event_maintains_status_history():
    ...
    assert history_messages == messages[-5:], "Should keep last 5 messages in FIFO order"

    # Verify timestamps are present and increasing
    timestamps = [ts for ts, msg in task.status_history]
    assert all(isinstance(ts, datetime) for ts in timestamps), "All timestamps should be datetime objects"
...
def test_on_circuit_breaker_sets_critical_event_flag():
    ...
    assert pipeline._critical_event.is_set(), "Critical event flag should be set after circuit breaker"
...
def test_wait_for_update_returns_true_on_critical_event():
    ...
    assert elapsed < 0.2, f"Should wake up quickly (got {elapsed:.3f}s), not wait full timeout"
...
def test_get_snapshot_returns_task_snapshot_dict():
    ...
    for task_id, task_snap in snapshot.items():
        assert isinstance(task_snap, TaskSnapshot), f"Task {task_id} should be TaskSnapshot"
        assert not isinstance(task_snap, URLTask), f"Task {task_id} should NOT be URLTask"
...
def test_get_snapshot_returns_deep_copy():
    ...
    assert original_task.progress_pct != 999.0, "Original task should not be affected by snapshot modification"

    # Get new snapshot and verify it still has original value
    new_snapshot = pipeline.get_snapshot()
    assert new_snapshot[1].progress_pct != 999.0, "New snapshot should have original value, not modified value"
...
def test_get_snapshot_includes_status_history():
    ...
    assert hasattr(snapshot[1], "status_history"), "Snapshot should include status_history"
    ...
    new_snapshot = pipeline.get_snapshot()
    assert len(new_snapshot[1].status_history) == 4, "Original history should be unchanged"
...
def test_concurrent_update_status_from_multiple_threads():
    ...
    assert dispatched_count == 100, f"Should have dispatched all 100 events (got {dispatched_count})"

    # Verify all tasks were updated (at least once)
    for task_id in range(1, 11):
        task = pipeline._tasks[task_id]
        assert task.state == URLState.PROCESSING, f"Task {task_id} should be in PROCESSING state"
        assert "Update from thread" in task.status_message, f"Task {task_id} should have update message"
...
def test_get_snapshot_during_concurrent_updates():
    ...
    for i, snapshot in enumerate(snapshots_taken):
        ...
        for task_id, task_snap in snapshot.items():
            assert isinstance(task_snap, TaskSnapshot), f"Snapshot {i} task {task_id} should be TaskSnapshot"
            assert 1 <= task_snap.task_id <= 20, f"Snapshot {i} task {task_id} should have valid task_id"
            assert 0.0 <= task_snap.progress_pct <= 100.0, f"Snapshot {i} task {task_id} should have valid progress"
...
def test_get_stats_returns_correct_counts():
    ...
    assert stats["active"] == 5, "Should have 5 active tasks (3 SCRAPING + 2 PROCESSING)"
    assert stats["complete"] == 2, "Should have 2 complete tasks"
    assert stats["failed"] == 1, "Should have 1 failed task"

    # Verify sum
    assert stats["pending"] + stats["active"] + stats["complete"] + stats["failed"] == stats["total"], "Stats should sum to total"
